...
export function buildCompactSlicePrompt(
  ...
  return [
    ...
    '- Emit the EXACT operation each behaviour NAMES for its gesture. When the vocabulary has similar ops, pick',
    '  the one the clause specifies for THAT gesture and never substitute a related-but-different op (e.g. an',
    '  inline mention is `create-reference`, NOT the structural `link`) — the wrong op silently does nothing',
    '  visible.',
    '- Implement the behaviour END-TO-END so its VISIBLE outcome appears, not just the op: the gesture → the op →',
    '  the resulting RENDER (the new/updated element actually shows) → any LIVE update the clause requires (e.g.',
    '  an inline reference shows the target\'s CURRENT label and refreshes on every keystroke that edits it).',
    '  If a behaviour offers choices (a picker), POPULATE them from the relevant state (e.g. existing nodes).',
    '',
