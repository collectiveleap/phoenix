...
export function generateProjectConfig(
  ...
  // A target that provisions a browser engine gets a Playwright config for the
  // rendered-ui surface. Phoenix owns the boot and passes the URL via env — so there
  // is deliberately NO `webServer` here. `*.ui.spec.ts` is outside vitest's `*.test.ts`
  // glob, so the two runners never collide.
  if (target?.runtime.browserDeps?.length) {
...
// AUTO-GENERATED by Phoenix VCS — the rendered-ui evaluation surface.
// Phoenix boots the app itself and passes its URL as PHOENIX_BASE_URL, so no
// Playwright-managed server is configured (Phoenix owns the boot).
export default defineConfig({
